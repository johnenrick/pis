<template >
  <div class="section">
    <div class="row">
      <div class="col-sm-3">
        <div class="">
          <label v-if="isLoading.length" class="control-label"><i  class="fa fa-hourglass-half" aria-hidden="true"></i> Loading...</label>
        </div>
      </div>
      <div class="col-sm-9 text-right">
          <select v-model="currentFilter">
            <template v-for="(filterSetting, index) in filterSettings">
              <option v-bind:value="index" v-bind:placeholder="filterSetting['placeholder']" v-bind:type="filterSetting['type']">{{filterSetting["name"]}}</option>
            </template>
          </select>
          <input v-model="filterValue" v-bind:name="filterSettings[currentFilter]['name']"   v-bind:placeholder="filterSettings[currentFilter]['placeholder']">
          <button v-on:click="retrieveData(1)" type="button" class="btn btn-default " v-bind:disabled="isLoading.length ? true : false"><i class="fa fa-search" aria-hidden="true"></i> Search</button>
          <button v-on:click="retrieveData(2)" type="button" class="btn btn-default " v-bind:disabled="isLoading.length ? true : false"><i class="fa fa-filter" aria-hidden="true"></i> Filter</button>
      </div>
    </div>
    <div class="row">
      <div class="col-sm-12">
        <table class="table  table-striped table-condensed table-bordered table-hover table-condensed table-responsive">
          <thead>
            <tr v-for="(columnSetting, parentIndex) in columnSettings">
                <th  v-for="(column, index) in columnSetting" v-on:click="changeSort(parentIndex, index,$event)" v-bind:rowspan="((typeof column['rowspan'] === 'undefined') ? 2 :  column['rowspan'])" v-bind:colspan="((typeof column['colspan'] === 'undefined') ? 1 :  column['colspan'])" v-bind:class="column['width_class']" >
                  {{column["name"]}}
              ...
              <tr  v-on:click="rowClicked(index)">
                <template v-for="col in linearColumnSettings">
                  <td v-if="col['db_name']" v-bind:class="'col-type-'+col['type']">
                    ...
                    <input v-else-if="col['type'] === 'checkbox'" type='checkbox' v-bind:value="col['db_name'] | dataFormat(col, row)" v-model="row[col['db_name']]" onclick="return false">
                    <span v-else >
    ...
    retrieveData(mode){
      ...
      if(mode === 1){//search
        ...
        if(mode === 2 && this.filterValue !== ""){//filter
          ...
          requestOption.params.condition = [condition]
        }
        this.$http.get(this.api.retrieve, requestOption).then((response) => {
          // success callback
          var result = response.body;
    ...
    initColumnSetting(){
      ...
      if(initializedData.length){//check if it has value
        if(initializedData[0].length === undefined){//check if object not array; one row thead
          ...
          for(;dsrCounter < initializedData[1].length; dsrCounter++){//create linear array
